...
options:
  ...
  cafile:
    description:
      - The optional path to a file containing CA certificates used to
        validate clients' certificates.
    type: str
  capath:
    description:
  ...
  hmac_algo:
    description:
      ...
      - See your python's hashlib.algorithms_available set for
        available options.
    type: str
    default: "sha256"
  hmac_header:
    ...
    if "certfile" in args:
        ...
        if cafile or capath:
            try:
                ...
                logger.exception(
                    "Failed to load CA certificates. Check they are valid"
                )
                raise
