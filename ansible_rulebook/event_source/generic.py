...
DOCUMENTATION = r"""
---
short_description: A generic source plugin that allows you to
                   insert custom data.
description:
...
options:
  ...
  event_delay:
    description:
      - The number of seconds to wait before injecting the next event
        from the payload.
    type: float
    default: 0
  repeat_delay:
    description:
      - The number of seconds to wait before injecting a repeated event
        from the payload.
    type: float
    default: 0
  loop_delay:
  ...
  loop_count:
    description:
      - The number of times the set of events in the payload
        should be repeated.
    type: int
    default: 1
  repeat_count:
    description:
      - The number of times each individual event in the payload
        should be repeated.
    type: int
    default: 1
  blob_size:
    description:
      - An arbitrary blob of blob_size bytes to be inserted into every
        event payload.
...
class EnvVarMismatchError(Exception):
    ...
    def __init__(
        ...
        super().__init__(
            f"Env Var {env_var} expected: {expected} passed in: {value}"
        )
...
class Generic:
    ...
    def _validate_time_format(self: Generic) -> None:
        """Validate the time format configuration."""
        if (
            self.control_args.timestamp
            and self.control_args.time_format
            not in [
                "local",
                "iso8601",
                "epoch",
    ...
    def _prepare_payload(self: Generic) -> None:
        """Prepare and normalize the payload."""
        if not isinstance(self.my_args.payload, list):
            self.my_args.payload = [self.my_args.payload]

    async def _process_event_repeats(
        self: Generic, event: dict[str, Any], index: int
    ) -> int:
        """Process repeated events and return updated index."""
    ...
    async def _process_payload_iteration(self: Generic, index: int) -> int:
        """Process one iteration of the payload and return updated index."""
        if self.control_args.randomize:
            random.shuffle(self.my_args.payload)  # NOSONAR(S2245)
        for event in self.my_args.payload:
            if not event:
                continue
            index = await self._process_event_repeats(event, index)
    ...
    async def __call__(self: Generic) -> None:
        ...
        while iteration != self.control_args.loop_count:
            ...
            index = await self._process_payload_iteration(index)
            iteration += 1
    ...
    def _check_env_vars(self: Generic) -> None:
    ...
    def _load_payload_from_file(self: Generic) -> None:
