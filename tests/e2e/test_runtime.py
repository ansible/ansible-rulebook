...
def test_mask_variables_debug_no_params():
    ...
    assert result.returncode == 0
    assert not result.stderr
    assert f"'postgres_db_password': '{MASKED_STRING}'" in result.stdout
    assert f"'tower_password': '{MASKED_STRING}'" in result.stdout
    assert f"'aap_password': '{MASKED_STRING}'" in result.stdout
    assert f"'controller_password': '{MASKED_STRING}'" in result.stdout
    assert f"'private_key': '{MASKED_STRING}'" in result.stdout
    assert f"'service1_password': '{MASKED_STRING}'" in result.stdout
    assert f"'service2_token': '{MASKED_STRING}'" in result.stdout
    assert f"'aap_passphrase': '{MASKED_STRING}'" in result.stdout
...
def test_mask_variables_debug_var_and_message():
    ...
    assert f"Test message override {MASKED_STRING}" in result.stdout
