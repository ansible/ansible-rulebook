...
async def test_noop():
    ...
    action_args = {"event": {"b": 1}, "ruleset": metadata.rule_set}

    with patch("uuid.uuid4", return_value=DUMMY_UUID):
        with patch("ansible_rulebook.action.run_job_template.lang.post") as drools_mock:
            await PostEvent(metadata, control, **action_args)()
            drools_mock.assert_called_once_with(action_args["ruleset"], action_args["event"])
    ...
    assert action["action"] == "post_event"
