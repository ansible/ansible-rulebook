...
def main(queue, args):
    ...
    if names is not None:
        original_processes = {pid: name for pid, name in original_processes.items() if name in names}

    while True:
        time.sleep(1)
        current_processes = {p.pid: p.name() for p in psutil.process_iter()}
        if names is not None:
            current_processes = {pid: name for pid, name in current_processes.items() if name in names}
        ...
        for new in new_processes:
            queue.put(dict(process_check=dict(pid=new,
                           name=current_processes[new],
                           status='running'
                           )))
        for lost in lost_processes:
            queue.put(dict(process_check=dict(event_type='lost_process',
                           pid=lost,
                           name=original_processes[lost],
                           status='stopped'
                          )))
